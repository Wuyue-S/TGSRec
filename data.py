...
class Data():
    ...
    def split_data(self, g_df, args):
        ...
        ts_l = g_df.ts.values
        #ill是疾病列表
        ill_l = g_df.ill.values


        max_src_index = src_l.max()
        self.max_idx = max(src_l.max(), dst_l.max())
        self.num_ill = ill_l.max()
        print('num_ill', self.num_ill)
        ...
        self.train_ill_l = ill_l[valid_train_flag]
        ...
        val_ill_l = ill_l[valid_val_flag]
        ...
        self.val_ill_l = val_ill_l[valid_is_old_node_edge]
        ...
        test_ill_l = ill_l[valid_test_flag]
        ...
        self.test_ill_l = test_ill_l[test_is_old_node_edge]
        print('#interaction in test: ', len(self.test_src_l))

        print('max_idx',self.max_idx)

        adj_list = [[] for _ in range(self.max_idx + 1)]
        for src, dst, eidx, ts, ill in zip(self.train_src_l, self.train_dst_l, self.train_e_idx_l, self.train_ts_l, self.train_ill_l):
            adj_list[src].append((dst, eidx, ts, ill))
            adj_list[dst].append((src, eidx, ts, ill))
        self.train_ngh_finder = NeighborFinder(adj_list, uniform=args.uniform)
        test_train_adj_list = [[] for _ in range(self.max_idx + 1)]
        for src, dst, eidx, ts, ill in zip(self.train_src_l, self.train_dst_l, self.train_e_idx_l, self.train_ts_l, self.train_ill_l):
            test_train_adj_list[src].append((dst, eidx, ts, ill))
            test_train_adj_list[dst].append((src, eidx, ts, ill))
        for src, dst, eidx, ts, ill in zip(self.val_src_l, self.val_dst_l, self.val_e_idx_l, self.val_ts_l, self.val_ill_l):
            test_train_adj_list[src].append((dst, eidx, ts, ill))
            test_train_adj_list[dst].append((src, eidx, ts, ill))
        ...
        for src, dst, eidx, ts, ill in zip(src_l, dst_l, e_idx_l, ts_l, ill_l):
            full_adj_list[src].append((dst, eidx, ts, ill))
            full_adj_list[dst].append((src, eidx, ts, ill))
